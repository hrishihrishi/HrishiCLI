...
import type { Prisma } from "@hrishicli/database";
import {
  getToolContracts,
  modeSchema,
  type ModeType,
  type ToolContracts,
} from "@hrishicli/shared";
...
type hrishicliUIMessage = UIMessage<
  ChatMessageMetadata,
  never,
...
const submitSchema = z.object({
  ...
  messages: z
    .array(
      z.custom<hrishicliUIMessage>((value) => {
        return (
          value != null &&
          typeof value === "object" &&
          "id" in value &&
...
function hasPendingToolCalls(message: hrishicliUIMessage) {
...
const app = new Hono<AuthenticatedEnv>().post(
  ...
  async (c) => {
    ...
    const previousMessages = Array.isArray(session.messages)
      ? (session.messages as unknown as hrishicliUIMessage[])
      : [];
    ...
    const nextMessages = await validateUIMessages<hrishicliUIMessage>({
    ...
    return result.toUIMessageStreamResponse<hrishicliUIMessage>({
